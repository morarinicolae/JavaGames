...
public class JocLabirint {
   ...
   JLayeredPane lp; 
    ...
    public JocLabirint() {
    	...
    	int i=-1,j=-1;
    	JFrame f=new JFrame("Andrei Bogdan-Marcel Joc Labirint");
     ...
     for(int i=0;i<7;i++) //linie
      for(int j=0;j<7;j++)  //coloana
         {g.drawRect(j*50,i*50,50,50);
          ...
          if(schema[i][j]=='1')g.setColor(Color.green);
          if(schema[i][j]=='0')g.setColor(Color.black);
 ...
 {l+=e.getY()/50; c+=e.getX()/50;
   setLocation(c*50,l*50);}}
    ...
    {super.paintComponent(g);
     ...
     try {im = ImageIO.read(new File("15624779-3d-cartoon-furry-monster.jpg"));} 
     catch (IOException e) {}
